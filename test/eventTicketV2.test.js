  ...
  describe("Functions", () => {
    describe("addEvent()", async () => {
      ...
      it("adding an event should emit an event with the provided event details", async () => {
        const tx = await instance.addEvent(
          event1.description,
          event1.website,
          event1.ticketsAvailable,
          { from: deployAccount }
        ...
        assert.equal(
          eventData.desc,
          event1.description,
          "the added event descriptions should match"
        );
        assert.equal(
          eventData.url,
          event1.website,
          "the added event Urls should match"
        );
        assert.equal(
          eventData.ticketsAvailable.toString(10),
          event1.ticketsAvailable.toString(10),
          "the added event ticket amounts should match"
        );
      });
    });
    ...
    describe("readEvent()", async () => {
      it("providing the event Id should return the correct event details", async () => {
        ...
        assert.equal(
          eventDetails["0"],
          event1.description,
          "the event descriptions should match"
        );
        assert.equal(
          eventDetails["1"],
          event1.website,
          "the website details should match"
        );
        assert.equal(
          eventDetails["2"].toString(10),
          event1.ticketsAvailable.toString(10),
    ...
    describe("buyTickets()", async () => {
      it("tickets should only be able to be purchased when the event is open", async () => {
        ...
        await instance.buyTickets(0, numberOfTickets, {
          from: firstAccount,
          value: ticketPrice
        });
        ...
        assert.equal(
          eventDetails["3"],
          numberOfTickets,
          `the ticket sales should be ${numberOfTickets}`
        );
      });
      ...
      it("tickets should only be able to be purchased when enough value is sent with the transaction", async () => {
        ...
        await catchRevert(
          instance.buyTickets(0, numberOfTickets, {
            from: firstAccount,
            value: ticketPrice - 1
          })
        );
      ...
      it("tickets should only be able to be purchased when there are enough tickets remaining", async () => {
        ...
        await instance.buyTickets(0, 51, {
          from: firstAccount,
          value: ticketPrice * 51
        });
        await catchRevert(
          instance.buyTickets(0, 51, {
            from: secondAccount,
            value: ticketPrice * 51
          })
        );
      ...
      it("a LogBuyTickets() event with the correct details should be emitted when tickets are purchased", async () => {
        ...
        const tx = await instance.buyTickets(0, numTickets, {
          from: firstAccount,
        ...
        assert.equal(
          eventData.eventId,
          0,
          "the event should have the correct eventId"
        );
        assert.equal(
          eventData.numTickets,
          numTickets,
          "the event should have the correct number of tickets purchased"
        );
      });
    });
    ...
    describe("getRefund()", async () => {
      it("only accounts that have purchased tickets should be able to get a refund", async () => {
        ...
        await instance.buyTickets(0, numTickets, {
          from: firstAccount,
          value: ticketPrice * numTickets
        });
        ...
        assert.equal(
          eventData.event,
          "LogGetRefund",
          "the event should be called LogGetRefund"
        );
        assert.equal(
          eventData.args.accountRefunded,
          firstAccount,
          "the firstAccount should be the 'accountRefunded'"
        );
      });
      ...
      it("account requesting a refund should be credited the appropriate amount", async () => {
        ...
        assert.equal(
          postSaleAmount.slice(-4),
          preSaleAmount.slice(-4),
    ...
    describe("getBuyerNumberTickets()", async () => {
      it("providing an event id to getBuyerNumberTickets() should tell an account how many tickets they have purchased", async () => {
        ...
        await instance.buyTickets(0, numberToPurchase, {
          from: secondAccount,
          value: ticketPrice * numberToPurchase
        });
        let result = await instance.getBuyerNumberTickets(0, {
          from: secondAccount
        });
        ...
        assert.equal(
          result,
          numberToPurchase,
          "getBuyerNumberTickets() should return the number of tickets the msg.sender has purchased."
        );
      });
    });
    ...
    describe("endSale()", async () => {
      it("only the owner should be able to end the sale and mark it as closed", async () => {
        ...
        assert.equal(
          eventData["4"],
          false,
          "The event isOpen variable should be marked false."
        );
      });
      ...
      it("endSale() should emit an event with information about how much ETH was sent to the contract owner", async () => {
        ...
        assert.equal(
          amount,
          ticketPrice * numberToPurchase,
          "the first emitted event should contain the tranferred amount as the second parameter"
        );
      });
    });
  });
