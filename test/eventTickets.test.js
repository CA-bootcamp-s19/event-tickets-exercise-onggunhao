  ...
  describe("Functions", () => {
    it("readEvent() should return myEvent details", async () => {
      ...
      assert.equal(
        event.totalTickets,
        ticketNumber,
        "the number of tickets for sale should be set"
      );
    ...
    describe("buyTickets()", async () => {
      ...
      it("tickets should only be able to be purchased when there are enough remaining", async () => {
        await instance.buyTickets(50, {
          from: secondAccount,
          value: ticketPrice * 50
        });
        await catchRevert(
          instance.buyTickets(51, {
            from: thirdAccount,
      ...
      it("the buyers account should be credited with tickets when they make a purchase", async () => {
        ...
        assert.equal(
          count,
          2,
          "the buyer should have 2 tickets in their account"
        );
      });
      ...
      it("buyers should be refunded any surplus funds sent with the transaction", async () => {
        ...
        assert.equal(
          (Number(postSaleAmount.slice(-4)) + ticketPrice).toString(),
          Number(preSaleAmount.slice(-4)).toString(),
    ...
    describe("getRefund()", async () => {
      ...
      beforeEach(async () => {
        preSaleAmount = await web3.eth.getBalance(secondAccount);
        await instance.buyTickets(1, {
          from: secondAccount,
          value: ticketPrice
        });
        result = await instance.getRefund({ from: secondAccount });
        postSaleAmount = await web3.eth.getBalance(secondAccount);
      });

      it("buyers should be refunded the appropriate value amount when submitting a refund", async () => {
        // console.log(preSaleAmount);
        // console.log(postSaleAmount);
      ...
      it("should emit a getRefund event", async () => {
        // console.log(result.logs[0]);
        const { requester, quantity } = result.logs[0].args;
        assert.equal(
          secondAccount,
          requester,
          "LogGetRefund event has incorrect requester property"
        );
        assert.equal(
          quantity,
          1,
          "LogGetRefund event has incorrect quantity property"
        );
      });

      it("should decrease the sales by quantity of refunded tickets", async () => {});

      it("should set the refund requesters number of tickets to 0", async () => {});
    });
    ...
    describe("endSale()", async () => {
      it("the event owner should be able to close ticket sales", async () => {
        ...
        assert.equal(
          eventDetails.isOpen,
          false,
          "ticket sales should be closed when the owner calls endSale()"
        );
      });
      ...
      it("tickets should be able to be purchased when the event is not open", async () => {
        await instance.endSale({ from: firstAccount });
        await catchRevert(
          instance.buyTickets(1, { from: secondAccount, value: ticketPrice })
        );
      });
